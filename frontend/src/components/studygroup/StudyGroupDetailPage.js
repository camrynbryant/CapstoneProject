      ...
      <div className="file-list">
        ...
        {loadingFiles ? (
          ...
          <>
            ...
            <ul>
              {files.map((file) => {
                const canDelete = currentUserEmail && file.uploaderEmail && currentUserEmail.toLowerCase() === file.uploaderEmail.toLowerCase();

                return (
                  <li key={file.id}>
                    <span>
                      {file.filename} ({(file.size / 1024).toFixed(1)} KB) - Uploaded by {file.uploaderEmail} on {new Date(file.uploadDate).toLocaleDateString()}
                    </span>
  ...
  useEffect(() => {
    const fetchGroup = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.message || err.response?.data || "Could not load group details. Please ensure you are logged in and the group exists.");
        setGroup(null);
      } finally {
        setLoading(false);
      }
      ...
      {loading ? (
        ...
        <>
          ...
          {isMember ? (
            ...
            <p style={{ fontStyle: 'italic', marginTop: '1rem' }}>Join the group to view resources and sessions.</p>
          )}
        </>
      ) : (
